...
#Region "Keyboard Handler"
    ...
    Private m_KeyTabEnabled As Boolean = True

    <Browsable(True), Category("Behavior"), DefaultValue(True), Description("Gets or sets if the CTRL+TAB/CTRL+SHIFT+TAB will select the next/previous tab.")> _
    Public Property KeyTabEnabled() As Boolean
        Get
    ...
    <Browsable(True), Category("Behavior"), DefaultValue(True), Description("Gets or sets if the CTRL+F4 will close the selected tab.")> _
    Public Property KeyCloseEnabled() As Boolean
        Get
    ...
    Public Class KeyHandledEventArgs
        Inherits HandledEventArgs
        Public Index As Integer
        Public Shift As Boolean = False
    End Class

    Public Event TabPressed(ByVal sender As Object, ByVal e As KeyHandledEventArgs)
    Public Event F4Pressed(ByVal sender As Object, ByVal e As KeyHandledEventArgs)

    Private Sub Owner_KeyDown(ByVal sender As Object, _
        ByVal e As System.Windows.Forms.KeyEventArgs)
        If TabPages.Count = 0 Then Exit Sub
        Dim ke As New KeyHandledEventArgs
        Dim Index As Integer
        If KeyTabEnabled And e.KeyCode = Keys.Tab And _
            e.Control And Not e.Alt Then
            If e.Shift Then
                Index = TabPages.SelectedIndex - 1
                If Index < 0 Then Index = TabPages.Count - 1
                ke.shift = True
            Else
                Index = TabPages.SelectedIndex + 1
                If Index > TabPages.Count - 1 Then Index = 0
            End If
            ke.Index = Index
            ke.Handled = False
            RaiseEvent TabPressed(Me, ke)
            If Not ke.Handled Then TabPages(Index).Select()
        ElseIf KeyCloseEnabled And e.KeyCode = Keys.F4 And _
            e.Control And Not e.Shift And Not e.Alt Then
            ke.Index = TabPages.SelectedIndex
            ke.Handled = False
            RaiseEvent F4Pressed(Me, ke)
            If Not ke.Handled Then TabPages.SelectedTab.m_Form.Close()
        End If
    End Sub
